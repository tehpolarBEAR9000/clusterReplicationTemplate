...
def combine(ec2,logDriver):
    ...
    for instance in instances:
        for groups in instance.security_groups:
            logDriver.instanceDataStorage([instance.id,groups['GroupId']])
            logDriver.ACLDataStorage(listSecurityGroupACLS(ec2.SecurityGroup(groups['GroupId']).ip_permissions))     
def listSecurityGroupACLS(sg):
    ...
    for rule in sg:
        for ip in rule['IpRanges']:
            return [(rule['FromPort'],rule['ToPort'],ip['CidrIp'])]
# storage class
class storageDriver():
    def __init__(self):
    ...
    def joinDataStorage(self):
        store = zip(self.instanceStorage,self.ACLStorage)
        for data in store:
            if data[1] != None:
                self.joinedDATAStore.append(data[0]+list(data[1][0]))
    def outputToCSV(self):
        ...
        with open('secGroupsPerInstance.csv', 'w') as fileOBJ:
...
#combine(ec2,logDriver)
#logDriver.joinDataStorage()
#print(logDriver.linkedMAP)
